...
error::Error UpdateModule::PrepareFileTree(const string &path, artifact::PayloadHeaderView &payload_meta_data) {
	...
	if (ec) {
		return error::Error(ec.default_error_condition(), "Could not clean File Tree for Update Module");
	}
	...
	err = CreateDataFile(
		header_subdir_path, "artifact_name", payload_meta_data.header.artifact_name);
	if (err != error::NoError) {
		return err;
	}

	err =
		CreateDataFile(header_subdir_path, "payload_type", payload_meta_data.header.payload_type);
	if (err != error::NoError) {
		return err;
	}
	...
	err =
		CreateDataFile(header_subdir_path, "type_info", payload_meta_data.header.type_info.verbatim.Dump());
	if (err != error::NoError) {
		return err;
	}
