...
// This state solves a tricky issue. When checking Update Control Maps before the reboot state, we
// want a few things to happen: If a pause is requested, we want a spontaneous reboot to act as if
// the update resumed after a normal reboot. This requires that we store datastore.MenderStateReboot
// in the database, so that the recovery logic will work. However, we can not store this *before*
// checking the Update Control Maps, because the action may resolve to "fail", and then it would be
// very bad to resume the update. So the maps need to be checked, and then, *only if* there is a
// pause, we enter the state below to store the datastore.MenderStateReboot in the database. Then we
// go back to pausing.
...
func (s *updateRebootPauseRequestedState) Handle(ctx *StateContext, c Controller) (State, bool) {
	// No need to fetch maps from the server. If we entered this state then it means we just
	// fetched them. Go directly to checking the action instead. Note that we also don't pass a
	// pauseState. Since we are in this state, the next time around, we want a real pause to
	// happen.
