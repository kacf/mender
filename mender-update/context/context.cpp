...
ExpectedProvidesData MenderContext::LoadProvides() {
	...
	mender_store_.ReadTransaction([this, &data](kv_db::Transaction &txn) {
	...
	for (auto to_clear : clears_provides) {
		...
		for (auto chr : to_clear) {
			if (chr == '*') {
				...
				// Escape every non-alpanumeric character except '*'.
				escaped.push_back('\\');
			}
		...
		regex compiled;
		auto err = common::ExceptionToErrorOrAbort([&compiled, &escaped]() {
			compiled.assign(escaped, regex_constants::basic);
		});
...
error::Error MenderContext::CommitArtifactData(
	...
	return mender_store_.WriteTransaction([&](kv_db::Transaction &txn) {
		...
		if (artifact_name != "") {
			err = txn.Write(artifact_name_key, common::ByteVectorFromString(artifact_name));
			if (err != error::NoError) {
				return err;
			}
		}
		if (artifact_group != "") {
			err = txn.Write(artifact_group_key, common::ByteVectorFromString(artifact_group));
			if (err != error::NoError) {
				return err;
			}
		}
