...
void Client::AsyncReadNextBodyPart(
	...
	size_t smallest = (body_buffer_.size() < read_size) ? body_buffer_.size() : read_size;
...
void Client::ReadBodyHandler(error_code ec, size_t num_read) {
	...
	if (response_->body_async_reader_) {
		...
		size_t smallest = (num_read < buf_size) ? num_read : buf_size;
	...
	if (response_->body_writer_ && num_read > 0) {
		...
		if (!written) {
			...
			CallErrorHandler(
				error::Error(error_condition(errc::io_error), "Short write when writing body"),
				request_,
				body_handler_);
			return;
		}
